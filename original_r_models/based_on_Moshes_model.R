...
# This step is for spectral resampling from ASD to FTNIR devices if needed.
# If the data is from FTNIR or it is necessary to work with ASD data, do not run this chunk.
...
# 1.1 Read the data from CSV file
data = read.csv("GLT_N_SPECT_FTNIR.csv", fileEncoding = "Latin1", check.names = F)
names(data)
nrow(data)
# 1.2 Identify and rename columns for classification, Y variables, and X variables (spectral data)
...
## Step 4:the models
# 4,1 Prepare data for modeling by joining spectral and chemical data
...
# Get all Spectra objects in the global environment
...
## Loop through each Y variable to create models
for (i in 1:ncol(Y_data)) {
  ...
  for (j in 1:length(objs)) {
    ...
    # minimize the cross-validation error, RMSPE
...
## Step 4: plot results  <---------------------------------------------------
# Plot observed vs. predicted values for calibration, validation, and prediction sets
names(obspred)
str(obspred)
...
# Summarize data by calculating RMSE, R-squared, RPD, and RPIQ for each variable and transformation
summerized_data = obspred_rm_outliers %>%
  group_by(variable, train_test) %>%
  summarise(RMSE = caret::RMSE(predicted,
                               observed),
...
# Read and transform additional data
data = read.csv("organized_FTNIR_data.csv")
